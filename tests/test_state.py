...
def test_records_exports_and_proposes(tmp_path: Path):
    ...
    assert "# Hermes Skill Automation Workflow Export — sess-1" in export_text
    ...
    assert "# Hermes Skill Automation Proposal Packet — sess-1" in proposal_text
...
def test_recent_and_last_session_views(tmp_path: Path):
    ...
    assert "Recent Hermes Skill Automation sessions" in recent
